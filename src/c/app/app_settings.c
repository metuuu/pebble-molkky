...
// "Other" (page header). Rows stay a flat list; the section map just groups them.
// (Named app_settings to avoid clashing with the keyboard's settings_window.)
// =============================================================================

static Menu *s_menu;

static uint16_t st_count(void *c) { return 3; }
static uint16_t st_sections(void *c) { return 2; }
static uint16_t st_section_count(void *c, uint16_t s) { return s == 0 ? 2 : 1; }
...
static void st_select(void *c, uint16_t i) {
  ...
  else if (i == 1) mk_set_final_round(!mk_final_round());
  else             mk_set_show_header(!mk_show_header());
  menu_reload(s_menu);
}
